...
const seatController = {
    async getSeats(req:Request,res:Response){
        try{
            ...
            const seats =  await seatServices.getSeats(screen_public_id);
    ...
    async getAvailableSeats(req:Request,res:Response){
        try{
            ...
            const available_seats =  await seatServices.getAvailableSeats(screen_public_id);
    ...
    async getSeatCount(req:Request,res:Response){
        try{
            ...
            const total_seats =  await seatServices.getSeatCount(screen_public_id);
    ...
    async bookSeat(req:Request,res:Response){
        try{
            const seat_public_id = req.params.screen_public_id as string;

            const booked = await seatServices.bookSeat(seat_public_id);
    ...
    async cancelSeat(req:Request,res:Response){
        try{
            ...
            const cancel = await seatServices.cancelSeat(seat_public_id);
    ...
    async holdSeat(req:Request,res:Response){
        try{
            ...
            const hold = await seatServices.holdSeat(seat_public_id);
    ...
    async freeSeats(req:Request,res:Response){
        try{
            const result = await seatServices.freeSeats();

            if(result)
                return res.status(200).json({message:"Seats freed"});
            else
                return res.status(409).json({message:"No seats to free"});
